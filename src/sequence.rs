...
impl Sequence {
    pub async fn new(seq_number: i32) -> Self {
    ...
    pub async fn register_cue(&mut self, cue_info: &CueInfo) -> anyhow::Result<(), anyhow::Error> {
        if self.cues.contains_key(cue_info.number.as_str()) {
            return Ok(());
        }

        let cue = Cue::new(self.id, cue_info).await;
    ...
    pub async fn process_fader(
        ...
        let message = rosc::OscMessage {
            addr: format!("/ma/seq/{}/{}", self.id, fader_functon),
            args: vec![OscType::Float(level)],
        };

        let packet = rosc::OscPacket::Message(message.clone());

        let buf = rosc::encoder::encode(&packet).expect("could not encode message");

        let send_res = socket.send_to(&buf, TOUCH);
        ...
        //     // self.faders.insert(short_id.clone());
        // }
