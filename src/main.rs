...
use rship_sdk::{InstanceArgs, InstanceProxy, SdkClient};
...
async fn handle_cues(
    ...
    sequence
        .register_cue(&cue_info, &instance)
        .await
...
async fn main() -> anyhow::Result<(), anyhow::Error> {
...

    {
        ...
        client.set_address(Some("192.168.1.224:5155".into()));
                ...
                for m in messages {
                    ...
                    if fader_matcher.match_address(&addr) {
                        ...
                        if !sequences.contains_key(&seq_number) {
                            ...
                            let sequence = Sequence::new(seq_number, &instance).await;
                        ...
                        match args[0] {
                            OscType::String(ref function) => {
                                let _ =
                                    handle_cues(function, &args, sequence, instance.clone()).await;

                                let _ = sequence.process_fader(function, &args).await;
